...
class TestImageHighLight(unittest.TestCase):
    def test_get_fingerprint(self):
        image_path = os.path.join('tests', 'test_data', '1904.png')
    ...
    def test_write(self):
        image_path = os.path.join('tests', 'test_data', '1905.png')
        guide_image_path = os.path.join('tests', 'test_data', '1904.png')
        output_image_path = os.path.join(
            'tests', 'test_data', 'test-highlight-output.png'
        )
        ImageHighlight(image_path, guide_image_path, 400).write(
            output_image_path
        )
